...
export default function AttributeEditable({
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const updatePayload = { [field]: type === 'number' ? Number(tempValue) : tempValue };
            await productAPI.update(productId, updatePayload);
            onUpdate(tempValue);
            showNotification(`Successfully updated ${field} to ${tempValue}`, 'success');
            setIsEditing(false);
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            {/* Inline Editor */}
            {isEditing && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full min-w-[200px] z-[601]">
                    <form
                        onSubmit={handleSave}
